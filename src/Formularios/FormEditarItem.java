...
public class FormEditarItem extends javax.swing.JFrame {
    ...
    public FormEditarItem(CatalogoPanel catalogoPanel, Items item) {
        this.catalogoPanel = catalogoPanel;
        this.item = item;
        
        initComponents();
        
        Image icon = Toolkit.getDefaultToolkit().getImage(FormEditarItem.class.getResource("/Imagenes/logoFlor.png"));
        setIconImage(icon);
    ...
    private void llenarCamposConDatos() {
        ...
        txtfNombreItem.setText(item.getNombreItem());
        txtfDescripcion.setText(item.getDescripcionItem());
        txtfPrecioProducto.setText(Float.toString(item.getPrecioProducto()));
        txtfExistencia.setText(Integer.toString(item.getExistencia()));

        cbTipoItem.setSelectedItem(item.getTipoItem());
        cbUnidad.setSelectedItem(item.getUnidadItem());
        
        // no se puede obtener la lista directamente de la base de datos porque entonces el objeto que se anadio
        // y el nuevo generado son diferentes instacias se necesita  Override equals() and hashCode() en esos modelos
        // pone el proveedor en el cb
        for (int i = 0; i < cbProveedores.getItemCount(); i++) {
            Proveedores p = cbProveedores.getItemAt(i);
            if (p != null && p.getIdProveedores()== item.getProveedores_idProveedores()) {
